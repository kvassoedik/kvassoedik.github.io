...
export async function getComics(email : string) {
    ...
    const date = new Date(Number(JSON.parse(JSON.stringify(await js)).year), Number(JSON.parse(JSON.stringify(await js)).month), Number(JSON.parse(JSON.stringify(await js)).day)).toLocaleDateString();
    
    return [JSON.parse(JSON.stringify(await js)).img, JSON.parse(JSON.stringify(await js)).safe_title, 
        date, JSON.parse(JSON.stringify(await js)).alt, formatDistanceToNowStrict(date) + " ago"];
}