...
async function getID(email) {
    ...
    let id = (await fetch(newUrl.url.href)).text();
    return id;
}
async function getComics(id) {
    ...
    let js = (await fetch(newUrl.url.href)).json();
...
document.getElementById("get-comics-btn").addEventListener("click", async () => {
    let email = document.getElementById("email-text").value;
    if (email.length <= 21 || email.slice(-21) != "@innopolis.university") {
        document.getElementById("error-msg").textContent = "Wrong email!";
    }
    else {
        document.getElementById("error-msg").textContent = "";
        let id = await getID(document.getElementById("email-text").value);
        if (!document.body.contains(document.getElementById("img-id"))) {
            let img = document.createElement("img");
            let text = document.createElement("p");
            let box = document.getElementById("box-comics");
            let comics = await getComics(id);
            img.src = comics[0];
            img.alt = comics[3];
            ...
            let date = new Date(Date.UTC(Number(comics[2][2]), Number(comics[2][1]), Number(comics[2][0]))).toLocaleDateString();
